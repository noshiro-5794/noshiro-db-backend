...
class CollectionItemUpdateItemSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if "order" not in attrs and "relation" not in attrs:
            raise serializers.ValidationError("order or relation is required.")
        return attrs
...
class CollectionDetailResponseSerializer(CollectionListResponseSerializer):
    """Detailed collection representation; currently identical to list items."""
