...
class UserSettingsUpdateRequestSerializer(serializers.Serializer):
    ...
    show_adult_content = serializers.BooleanField(required=False)
    confirm_adult_content = serializers.BooleanField(required=False, write_only=True)

    def validate(self, attrs):
        if not attrs:
            raise serializers.ValidationError("No fields to update.")
        if attrs.get("show_adult_content") and not attrs.get("confirm_adult_content"):
            raise serializers.ValidationError(
                {
                    "confirm_adult_content": (
                        "Explicit confirmation is required to enable adult content."
                    )
                }
            )
        attrs.pop("confirm_adult_content", None)
        return attrs
