...
def import_anilist_media_task(
    ...
    *,
    ...
    sync_job_service.mark_running(
        job_id=job_id,
        total_count=1,
        current_label=f"Importing AniList {anilist_id}",
        lease_owner=lease_owner,
    )
    try:
        entity = anilist_import_service.import_media(anilist_id)
        result = {
        ...
        sync_job_service.advance(
            ...
            processed=1,
            synced=1,
            current_label=f"Imported AniList {anilist_id}",
            lease_owner=lease_owner,
        )
