...
def test_full_episode_sync_filters_by_bangumi_source() -> None:
    with patch("apps.sync.tasks.full_sync.Subject.objects.filter") as filter_subjects:
        filter_subjects.return_value.exists.return_value = False
        FullEpisodeSyncTask().sync_one(123)

    filter_subjects.assert_called_once_with(
...
def test_sync_job_list_query_rejects_unknown_filters() -> None:
    serializer = SyncJobListRequestSerializer(
        data={"status": "stuck", "job_type": "everything"}
    )

    assert not serializer.is_valid()
    assert set(serializer.errors) == {"status", "job_type"}
